...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        cur_place, i = self.place, 0
        while cur_place is not beehive:
            rand_bee = rANTdom_else_none(cur_place.bees)
            if self.min_range <= i <= self.max_range and rand_bee:
                return rand_bee
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        ant_armor = self.armor

        Ant.reduce_armor(self, amount)
        damage = amount
        if ant_armor-damage <= 0:
            damage += self.damage
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.digesting > 0:
            ...
            rand_bee = rANTdom_else_none(self.place.bees)
            if rand_bee:
...
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if len(QueenAnt.queens > 1):
            for i in range(1, len(QueenAnt.queens)):
                ScaryThrower.reduce_armor(QueenAnt.queens[i], QueenAnt.queens[i].armor)

        ScubaThrower.action(self, gamestate)
        cur_place = self.place.exit
        while cur_place:
            ant = cur_place.ant
            if ant and ant not in QueenAnt.buffed_ants:
                ant.damage *= 2
