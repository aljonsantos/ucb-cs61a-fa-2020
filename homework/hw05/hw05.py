class VendingMachine:
    ...
    def vend(self):
        res = ''
        if self.stock <= 0:
            res = 'Inventory empty. Restocking required.'
        elif self.funds < self.price:
            res = 'You must add ${0} more funds.'.format(self.price-self.funds)
        elif self.funds == self.price:
            self.funds -= self.price
            self.stock -= 1
            res = 'Here is your {0}.'.format(self.name)
        else:
            res = 'Here is your {0} and ${1} change.'.format(self.name, self.funds-self.price)
            self.funds = 0
            self.stock -= 1
        return res
