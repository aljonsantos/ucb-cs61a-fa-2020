...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while num_rolls > 0:
        ...
        if outcome == 1:
            ...
            sum_outcome += outcome
        num_rolls -= 1
    return 1 if pig_out else sum_outcome
    # END PROBLEM 1
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while scores[0] < goal and scores[1] < goal:
        ...
        while True: 
            ...
            num_rolls = strategy(scores[player], scores[opponent])

            scores[player] += take_turn(num_rolls, scores[opponent], dice)
            ...
            if (scores[0] >= goal or scores[1] >= goal) or not extra_turn(scores[player], scores[opponent]): 
                break
        who = opponent
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        player_current_score = score0 if who == 0 else score1
        player_last_score, player_running_high = last_score, running_high

        gain = player_current_score - player_last_score
        if gain > player_running_high:
            print(gain, "point(s)! The most yet for Player", who)
            player_running_high = gain
        player_last_score = player_current_score
        return announce_highest(who, player_last_score, player_running_high)
    return say
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    max_dice, max_average = 1, 0
    averaged_roll_dice = make_averaged(roll_dice, trials_count)
    
    nth_dice = 1
    while nth_dice <= 10:
        average = averaged_roll_dice(nth_dice, dice)
        if (average > max_average):
            max_average, max_dice = average, nth_dice
        nth_dice += 1
    return max_dice
    # END PROBLEM 9
